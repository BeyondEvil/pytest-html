...
const getOutcome = ({ nodeid }, tests) => {
    const relatedOutcome = tests.filter((test) => test.nodeid === nodeid).map(({ outcome }) => outcome)
...
const renderContent = (tests) => {
    ...
    const rows = renderSet.map((test) =>dom.getResultTBody(test, getOutcome(test, tests)))
